 ...
 *
 */
!function(a){var c=jsless.console;var b={settings:{behavior:{name:null,event:"click",eventSource:"self",target:"self",delay:-1,params:[]}},behaviors:{execute:function(f,d,g,i){var k=a.extend(true,{name:"execute",object:"jQuery",method:null},jsless.settings.behavior,i.behavior,g);if(!k.method){c.error("execute method not specified: "+JSON.stringify(k))}var j=k.params;var e=jsless.getSelector(k.eventSource,f,d).getVal();var l=jsless.getSelector(k.target,f,d);var h=function(o){c.debug(k.name+" event:"+k.event+" method: "+k.method+"\r\n\t :: "+JSON.stringify(k));var p=d.triggerHandler("jsless-"+k.name+"-begin");if(p===undefined||p){var m=l.getVal();var n=function(){d.triggerHandler("jsless-"+k.name+"-beforecomplete");var r=m;var q=m[k.method];if(k.object!="jQuery"){j.unshift(m);var r=window;a.each(k.object.split("."),function(t,u){r=r[u]});var q=r[k.method]}var s=q.apply(r,j);d.triggerHandler("jsless-"+k.name+"-complete")};if(k.delay<0){n()}else{setTimeout(n,k.delay)}}};if(k.event=="load"){f.one("jsless-widget-complete",h)}else{d.bind(k.event,h)}},keyclick:function(f,d,g,h){var j=a.extend(true,{name:"keyclick",event:"keydown",target:"self",keycode:13},jsless.settings.behavior,h.behavior,g);var i=j.params;var e=jsless.getSelector(j.eventSource,f,d).getVal();var k=jsless.getSelector(j.target,f,d);e.bind(j.event,function(n){c.debug(j.name+" event:"+j.event+"\r\n\t :: "+JSON.stringify(j));var o=d.triggerHandler("jsless-"+j.name+"-begin");if(o===undefined||o){var m=(n.keyCode?n.keyCode:n.which);if(m==j.keycode){n.preventDefault();var l=k.getVal();l.triggerHandler("click")}}})},toggleClass:function(f,d,g,h){var j=a.extend(true,{name:"toggleClass",className:null},jsless.settings.behavior,h.behavior,g);if(!j.className){c.error("className not specified: "+JSON.stringify(j))}var i=j.params;var e=jsless.getSelector(j.eventSource,f,d).getVal();var k=jsless.getSelector(j.target,f,d);e.bind(j.event,function(o){c.debug(j.name+" event:"+j.event+"\r\n\t :: "+JSON.stringify(j));var p=d.triggerHandler("jsless-"+j.name+"-begin");if(p===undefined||p){var m=k.getVal();var l=a(o.target).parents(j.target).addBack(j.target).first();var n=j.className;m.removeClass(n);l.addClass(n)}})}}};c.info("Loading Behaviors ...");window.jsless=a.extend(true,b,window.jsless||{})}(window.jQuery);