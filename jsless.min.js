 ...
 *
 */
!function(a){var c=jsless.console;var b={settings:{method:{name:null,onSuccess:"widget",onFail:"widget",params:{dynamic:{},forms:[]}}},getSelector:function(h,f,d){if(h==null){throw"selector is null"}var e=null;var i={target:null,scope:"widget",latebind:true,mode:"html",getVal:function(){return e}};if(typeof h==="object"&&!(h instanceof jQuery)){a.extend(i,h)}else{i.target=h}var g=function(){var j=null;if(i.scope=="widget"){j=f}else{if(i.scope=="self"){j=d}else{j=a(i.scope)}}var k=j;if(i.target!=null){k=j.find(i.target)}return k};if(h=="self"){e=d}else{if(h=="widget"){e=f}else{if(i.latebind){i.getVal=g}else{e=g()}}}return i},compileParams:function(h,e,d){var i=a.extend({},h);if(i.dynamic){var f={};a.each(i.dynamic,function(j,l){var k={target:l,object:"jQuery",method:"val",methodparams:[]};if(typeof l==="object"&&!(l instanceof jQuery)){a.extend(k,l)}k=jsless.getSelector(k,e,d);f[j]=function(){var m=k.getVal();var n=m;if(k.object!="jQuery"){n=window[k.object]}var o=n[k.method].apply(m,k.methodparams);return o}});i.dynamic=f}if(i.forms){var g=[];a.each(i.forms,function(k,l){var j=jsless.getSelector(l,e,d);g.push(j)});i.forms=g}return i},getParams:function(d){var g=a.extend({},d);var e={};if(g.dynamic){a.each(g.dynamic,function(j,k){e[j]=k()});delete g.dynamic}var f={};if(g.forms){a.each(g.forms,function(m,k){var j=k.getVal();var l=jsless.processContainer(j);a.extend(f,l)});delete g.forms}var i={};a.each(g,function(j,l){var k=j;if(j.indexOf("_dynamic")>0||j.indexOf("_forms")>0){k=j.substr(1)}i[k]=l});var h=a.extend({},f,e,i);return h},getValue:function(d,f,e){if(d.attr("contenteditiable")!=null){f[e]=d.html()}else{if(d.is("select[multiple] option:selected")){f[e]=f[e]||[];f[e].push(d.val())}else{if(d.is("input[data-list]:checkbox")){f[e]=f[e]||[];if(d.is(":checked")){f[e].push(d.val())}}else{if(d.is("input:checkbox,input:radio")){if(d.is("input[value]")){if(d.is(":checked")){f[e]=d.val()}}else{f[e]=d.is(":checked")}}else{if(d.is("option:selected")){f[e]=d.val()}else{if(d.is("select")){d.find("option:selected").each(function(g,h){f=jsless.getValue(a(h),f,e)})}else{f[e]=d.val()}}}}}}return f},processContainer:function(e){var h="[name],[data-list]";var j="input[type!='button'][type!='submit'],select,textarea,[contenteditable]";var f=e.find(h+","+j).filter(function(k){var l=a(this).parentsUntil(e,h).length==0;return l});var g=f.filter(j);var d=f.not(g);var i={};a.each(g,function(m,l){var k=a(l);var n=k.attr("name");if(k.attr("data-list")){n=k.attr("data-list")}else{if(k.attr("data-index")!=null){n+="["+k.attr("data-index")+"]"}}if(n==null){c.warn("element has no name: "+k[0].outerHTML);return}var o=jsless.getValue(k,i,n);i=o});a.each(d,function(m,l){var k=a(l);var o=jsless.processContainer(k);if(k.attr("data-list")!=null){var n=k.attr("data-list");i[n]=i[n]||[];i[n].push(o)}else{var n=k.attr("name");if(k.attr("data-index")!=null){n+="["+k.attr("data-index")+"]"}i[n]=o}});return i},_methods:{html:function(e,d,f,j){var k=a.extend(true,{name:"html",url:null,method:"GET",event:"click",eventstop:false,onSuccess:"widget",onFail:null,params:{dynamic:{},forms:[]}},jsless.settings.method,j.method,f);if(!k.url){c.error("html url not specified: "+JSON.stringify(k))}if(!k.onFail){k.onFail=k.onSuccess}var l=jsless.getSelector(k.onSuccess,e,d);var h=jsless.getSelector(k.onFail,e,d);var g=jsless.compileParams(k.params,e,d);var i=function(o){c.debug(k.name+" event:"+k.event);if(k.eventstop&&k.event!="load"){o.preventDefault()}var p=jsless.getParams(g);var n=l.getVal();var m=h.getVal();var q=d.triggerHandler("jsless-"+k.name+"-begin");if(q===undefined||q){d.trigger("jsless-ajax-begin");jsless.invoke({url:k.url,category:"normal",datatype:"json",method:k.method,subdomain:null,params:p,onComplete:function(t){t.success=t.success&&t.isHTML;var r=a(t.data);if(r.attr("data-jsless-error")){t.success=false;t.errormessage=r.attr("data-jsless-error")}var u=l;var s=n;if(!t.success){u=h;s=m;d.trigger("jsless-ajax-beforefail");s.triggerHandler("jsless-"+k.name+"-beforefail")}else{d.trigger("jsless-ajax-beforesuccess");s.triggerHandler("jsless-"+k.name+"-beforesuccess")}d.trigger("jsless-ajax-beforecomplete");s.triggerHandler("jsless-"+k.name+"-beforecomplete");a.each(s,function(y,x){var w=a(x);var v=r.clone();w[u.mode](v);v.jsless(j)});if(t.success){d.trigger("jsless-ajax-success");s.triggerHandler("jsless-"+k.name+"-success")}else{d.trigger("jsless-ajax-fail");s.triggerHandler("jsless-"+k.name+"-fail")}d.trigger("jsless-ajax-complete");s.triggerHandler("jsless-"+k.name+"-complete")},retryCount:0})}};if(k.event=="load"){e.one("jsless-widget-complete",i)}else{d.bind(k.event,i)}},htmlform:function(f,d,g,h){var i=a.extend(true,{name:"html",url:null,method:"GET",event:d.is("form")?"submit":"click",eventstop:d.is("form"),onSuccess:"widget",onFail:null,params:{dynamic:{},forms:[]}},jsless.settings.method,h.method,g);if(i.params.forms.length==0){if(d.is("form")){c.debug("element is form");i.params.forms.push(d)}else{var e=d.parentsUntil(f,"form");if(e.length==1){c.debug("element is within form")}else{c.debug("element is formless");e=d}i.params.forms.push(e)}}jsless._methods.html(f,d,i,h)}}};c.info("Loading Methods ...");window.jsless=a.extend(true,b,window.jsless||{})}(window.jQuery);/*
 ...
 *
 */
!function(a){var c=jsless.console;var b={settings:{behavior:{name:null,event:"click",eventSource:"self",target:"self",delay:-1,params:[]}},behaviors:{execute:function(f,d,g,i){var k=a.extend(true,{name:"execute",object:"jQuery",method:null},jsless.settings.behavior,i.behavior,g);if(!k.method){c.error("execute method not specified: "+JSON.stringify(k))}var j=k.params;var e=jsless.getSelector(k.eventSource,f,d).getVal();var l=jsless.getSelector(k.target,f,d);var h=function(o){c.debug(k.name+" event:"+k.event);var p=d.triggerHandler("jsless-"+k.name+"-begin");if(p===undefined||p){var m=l.getVal();var n=function(){d.triggerHandler("jsless-"+k.name+"-beforecomplete");var r=m;var q=m[k.method];if(k.object!="jQuery"){j.unshift(m);var r=window;a.each(k.object.split("."),function(t,u){r=r[u]});var q=r[k.method]}var s=q.apply(r,j);d.triggerHandler("jsless-"+k.name+"-complete")};if(k.delay<0){n()}else{setTimeout(n,k.delay)}}};if(k.event=="load"){f.one("jsless-widget-complete",h)}else{d.bind(k.event,h)}}}};c.info("Loading Behaviors ...");window.jsless=a.extend(true,b,window.jsless||{})}(window.jQuery);