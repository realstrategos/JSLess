 ...
 *
 */
!function(a){var c=jsless.console;var b={settings:{behavior:{name:null,event:"click",eventSource:"self",target:"self",delay:-1,params:[]}},behaviors:{execute:function(f,d,g,i){var k=a.extend(true,{name:"execute",object:"jQuery",method:null},jsless.settings.behavior,i.behavior,g);if(!k.method){c.error("execute method not specified: "+JSON.stringify(k))}var j=k.params;var e=jsless.getSelector(k.eventSource,f,d).getVal();var l=jsless.getSelector(k.target,f,d);var h=function(o){c.debug(k.name+" event:"+k.event+" method: "+k.method);var p=d.triggerHandler("jsless-"+k.name+"-begin");if(p===undefined||p){var m=l.getVal();var n=function(){d.triggerHandler("jsless-"+k.name+"-beforecomplete");var r=m;var q=m[k.method];if(k.object!="jQuery"){j.unshift(m);var r=window;a.each(k.object.split("."),function(t,u){r=r[u]});var q=r[k.method]}var s=q.apply(r,j);d.triggerHandler("jsless-"+k.name+"-complete")};if(k.delay<0){n()}else{setTimeout(n,k.delay)}}};if(k.event=="load"){f.one("jsless-widget-complete",h)}else{d.bind(k.event,h)}}}};c.info("Loading Behaviors ...");window.jsless=a.extend(true,b,window.jsless||{})}(window.jQuery);